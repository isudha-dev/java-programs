...
public class Problems12_140124 {
...
//        System.out.println(bitonicIndex(new int[] {3, 9, 10, 12, 20, 17, 5, 1}, 5));
//        sqrt(2147483647);
        ArrayList<ArrayList<String>> A = new ArrayList<>();
        A.add(new ArrayList<>(Arrays.asList("kqu","90")));
        A.add(new ArrayList<>(Arrays.asList("qkcb","10")));
        A.add(new ArrayList<>(Arrays.asList("qkcb","68")));
        A.add(new ArrayList<>(Arrays.asList("kqu","89")));
        A.add(new ArrayList<>(Arrays.asList("kqu","32")));
        A.add(new ArrayList<>(Arrays.asList("qkcb","28")));
        A.add(new ArrayList<>(Arrays.asList("qkcb","63")));
        A.add(new ArrayList<>(Arrays.asList("kqu","90")));
        A.add(new ArrayList<>(Arrays.asList("kqu","47")));
        System.out.println(highestScore(A));
    }
    ...
    public static int highestScore(ArrayList<ArrayList<String>> A) {
        ...
        for(ArrayList<String> arr: A) {
            map1.put(arr.getFirst(), map1.getOrDefault(arr.getFirst(), 0)+Integer.parseInt(arr.getLast()));
            map2.put(arr.getFirst(), map2.getOrDefault(arr.getFirst(), 0)+1);
        }
        ...
        for(Map.Entry<String , Integer> entry: map1.entrySet()){
            ...
            int count = map2.get(student);
            int avg = (int) Math.floor((double) marks / (double) count);
    ...
    public double findMedianSortedArrays1(final List<Integer> a, final List<Integer> b) {
        ...
        while(!validPartition && (arr1Size<a.size() || arr1Size<mid)){
            int arr2Size = l-arr1Size;
            if(arr1Size>0){
                l1 = a.get(arr1Size);
                r1 = a.get(arr1Size+1);
            }
    ...
    public static int bitonicIndex(int[] A, int B) {
        int n = A.length;
        int l = 0, r = n-1;
        int bitonicIdx = -1;
        while(l<=r){
        ...
        if(bitonicIdx == -1 || A[bitonicIdx] == B){
            return bitonicIdx;
        }
